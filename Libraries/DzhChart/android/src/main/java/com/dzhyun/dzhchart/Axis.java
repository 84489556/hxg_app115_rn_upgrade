...
public class Axis {
    ...
    public Axis(double minValue, double maxValue, double lastPrice, double height, int row){
        if (row==0) {
            threeAxis(minValue, maxValue, lastPrice);
            return;
        }
    ...
    public void threeAxis(double minValue, double maxValue, double lastPrice){
        if (minValue > maxValue){
            minValue = lastPrice*0.9;//跌停价
            maxValue = lastPrice*1.1;//涨停价
        }
        ...
        this.maxValue = lastPrice + split;
        this.minValue = lastPrice - split;
        this.stepValue = split;

        this.axises.add(this.minValue);
        this.axises.add(lastPrice);
        this.axises.add(this.maxValue);
    }
    ...
    public Axis(double minValue, double maxValue){
        ...
        double split = (maxValue - minValue) / 2;
    ...
    private void oldAxis(double minValue, double maxValue, double height) {
        if (minValue > maxValue){
            minValue = 0;
            maxValue = 0;
        }
        ...
        double[] fixedUnits = new double[]{10};
    ...
    public void newAxis(double minValue, double maxValue, double height){
...
//        }
        ...
        this.axises = new ArrayList<>();
        this.axises.add(minValue);
        if (minValue < 0) {
            this.axises.add(0.0);
        }
        this.axises.add(maxValue);

        this.minValue = this.axises.get(0);
        this.maxValue = this.axises.get(this.axises.size() - 1);

        if (minValue < 0) this.stepValue = (this.maxValue - this.minValue) /2;
        else this.stepValue = this.maxValue - this.minValue;
    }

    public Axis(double minValue, double maxValue, double height) {
        newAxis(minValue, maxValue, height);
    }
