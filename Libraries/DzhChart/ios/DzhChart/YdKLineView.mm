...
- (void)initSubViews {
    // ScrollView
    chartPV = [[YdChartPanelView alloc]init];
    chartPV.parentView = self;
    chartPV.chartView.parentView = self;
    [self addSubview:chartPV];
...
- (void)layoutSubviews {
    ...
    float containerWidth=chartPV.chartView.kLineWidth*(self.drawdata.count+.5), containerHeight=top+height;
    chartPV.frame = CGRectMake(0, 0, self.bounds.size.width, containerHeight);
    chartPV.contentSize = CGSizeMake(containerWidth, containerHeight);
//    chartPV.chartView.frame = CGRectMake(0, 0, containerWidth, containerHeight);
    chartPV.chartView.frame = CGRectMake(0, 0, self.frame.size.width/(SHOW_KLINE_MIN_COUNT-1)*(self.drawdata.count+.5), containerHeight);

    if (chartPV.isFirstShow) {
        chartPV.contentOffset = CGPointMake((containerWidth-self.bounds.size.width<0?0:containerWidth - self.bounds.size.width), 0);
    }
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:@"KLineCrossNotification" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull notification) {
        ...
        chartPV.chartView.legendPos = intCurKlineIndex;
    }];

}

- (void)updateAxisLabel {
    NSInteger drawDataCount = chartPV.chartView.showCount;
    if (drawDataCount > 0 && self.drawdata.count > 0 && chartPV.chartView.startPos < self.drawdata.count) {
        klineVolTime1.text = ((YdKLineStick*)self.drawdata[chartPV.chartView.startPos]).time;
        klineVolTime2.text = ((YdKLineStick*)self.drawdata[(chartPV.chartView.startPos+chartPV.chartView.endPos + 1)/2]).time;
        klineVolTime3.text = ((YdKLineStick*)self.drawdata[chartPV.chartView.endPos]).time;
...
- (void)setChartData:(NSString *)chartData {
    ...
    [self parseData:^(NSData *data) {
        ...
        if (chartPV.isFirstShow) {
            chartPV.chartView.startPos = (self.drawdata.count-SHOW_KLINE_COUNT)>0 ? (self.drawdata.count-SHOW_KLINE_COUNT) : 0;
            chartPV.chartView.showCount = SHOW_KLINE_COUNT;
        }
        
        [weakSelf updateFormulaData];
        
        if (chartPV.chartView.startPos > chartPV.chartView.endPos)
            return;
        
        [weakSelf calcAxis];
        [weakSelf updateAxisLabel];
        //先要计算出chartView 的frame，width为0是不会drawrect；所以没有使用 [chartPV.chartView setNeedsDisplay];
...
- (void)calcAxis {
    ...
    if (chartPV.chartView.showCount == 0 || chartPV.chartView.startPos > chartPV.chartView.endPos) return;
    ...
    for (NSInteger i = chartPV.chartView.startPos; i <= chartPV.chartView.endPos; ++i) {
    ...
    // 公式极值
    double max, min;
    mainFormula->getResult().min_max_in_range(chartPV.chartView.startPos, chartPV.chartView.endPos, max, min);
    kMax = MAX(kMax, max);
    kMin = MIN(kMin, min);
...
//    CGFloat fontHeight = SystemFontHeight(FONT_SIZE1);
    ...
    viceFormula->getResult().min_max_in_range(chartPV.chartView.startPos, chartPV.chartView.endPos, max, min);
    if (min == invalid_dbl) {
        min = 0;
    }
    self.viceMin = min;
...
//        self.viceMax = absVal;
    ...
    viceFormula1->getResult().min_max_in_range(chartPV.chartView.startPos, chartPV.chartView.endPos, max, min);
    if (min == invalid_dbl) {
        min = 0;
    }
    self.vice1Min = min;
...
//        self.vice1Max = absVal;
    ...
    chartPV.chartView.kLineWidth = self.frame.size.width/(chartPV.chartView.showCount-1);//减一根，半根显示的情况
}
...
- (void)setChartLoc:(NSString *)cl {
    ...
    if (chartPV.chartView.startPos > chartPV.chartView.endPos) return;
...
- (void)setMainName:(NSString*)fmlName {
    ...
    [self updateAxisLabel];
     ...
     [chartPV.chartView setNeedsDisplay];
//    [self setNeedsDisplay];
//    [self setNeedsLayout];
//    [self layoutIfNeeded];
}
...
- (void)updateLegend {
    NSInteger cursor = chartPV.chartView.legendPos == -1 ? chartPV.chartView.endPos : min(chartPV.chartView.endPos, chartPV.chartView.legendPos);
    if (cursor == -1) return;
