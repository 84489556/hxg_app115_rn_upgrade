...
//        _result.push_back(FormulaResult(wzkp));
    ...
    }
    {
        ...
        Line3->_drawPositon1 = __IF(C >= __HHV(C,20) && 控盘 > 0, 控盘, make_vector(size, 0));
    ...
    }
    {
        ...
        gdkp->_name = "";//高度控盘
        gdkp->_data = __IF(C >= __HHV(C,20) && 控盘 > 0, 控盘, make_vector(size, 0));
        gdkp->_nodraw = true;
...
const FormulaResults& DKZJ::run() {
    ...
    vector<double> vecIn(_sticksFundFlow.size(), invalid);
    vector<double> vecOut(_sticksFundFlow.size(), invalid);
...
//    double hugeOut5=0;
    ...
    for (int i=0; i<_sticksFundFlow.size(); i++) {
        ...
        if(ff.largeIn == invalid){
            ff.largeIn=0;
        }
        if(ff.largeOut == invalid){
            ff.largeOut=0;
        }
        if(ff.superIn == invalid){
            ff.superIn=0;
        }
        if(ff.superOut == invalid){
            ff.superOut=0;
        }
        
        
        vecIn[i]  = (ff.largeIn+ff.superIn);
        vecOut[i] = (ff.largeOut+ff.superOut);


        
//        hugeIn5 += (ff.largeIn != invalid) ? ff.largeIn : 0;
//        hugeIn5 += (ff.superIn != invalid) ? ff.superIn : 0;
//        hugeOut5 += (ff.largeOut != invalid) ? ff.largeOut : 0;
//        hugeOut5 += (ff.superOut != invalid) ? ff.superOut : 0;
    }

    const int color1 = 0xEB3323;
    const int color2 = 0x71FA4C;
    const int color3 = 0x580D18;
    const int color4 = 0x265E22;
...
//    size_t size = _sticksFundFlow.size();


    shared_ptr<FormulaLine> totalIn = make_shared<FormulaLine>();
    totalIn->_name = "主力流入";
    totalIn->_color = color1;
//    totalIn->_type = COLORSTICK;
    totalIn->_nodraw = true;
//    totalIn->_thick = -1;
    totalIn->_data = vecIn;
    _result.push_back(FormulaResult(totalIn));
    
    shared_ptr<FormulaLine> totalOut = make_shared<FormulaLine>();
    totalOut->_name = "主力流出";
    totalOut->_color = color2;
    totalOut->_nodraw = true;
//    totalOut->_thick = -1;
    totalOut->_data = vecOut;
    _result.push_back(FormulaResult(totalOut));
...
//    }
...

    {
        shared_ptr<FormulaDraw> D1 = make_shared<FormulaDraw>();
        D1->_type  = DRAWTEXT;
        D1->_color = 0xF5860F;//COLORYELLOW;
        D1->_drawPositon1 = AY==5;
        D1->_drawPositon2 = L*0.98;
        D1->_text = "5";
        _result.push_back(FormulaResult(D1));
...
const FormulaResults& ZLZJ5::run() {
...
//资金雷达(5日主力资金，五期资金)
    ...
    for (int i=0; i<_sticksFundFlow.size(); i++) {
        ...
        if (i>=4) {
            for (int j=0; j<5; j++) {
                FundFlowStick & ff = _sticksFundFlow.at(i-j);
                if (ff.hugeIn != invalid && ff.superOut != invalid){
                    hugeIn5  += ff.hugeIn;
                    hugeOut5 += ff.hugeOut;
                }
            }
        ...
        if (hugeIn5 != invalid && hugeOut5 != invalid){
            value = (hugeIn5-hugeOut5);
        }
...
//    }
    ...
    
    {
        ...
        shared_ptr <FormulaDraw> D1 = make_shared<FormulaDraw>();
        D1->_name = "净额";   // 资金净值
        D1->_drawPositon1 = vec;
        D1->_drawPositon3 = make_vector(size, 0);
        D1->_type = CURVESHADOW;
        D1->_color = 0xF9FF00;
        D1->_color2 = 0xF61F4E;
        D1->_color3 = 0x44E8CE;
       _result.push_back(FormulaResult(D1));
