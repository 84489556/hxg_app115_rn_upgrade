...
@property(nonatomic, copy) NSDictionary *hotBlock;//{lock:left,hot:{x:,y:,w:,h:}}
...
- (instancetype)initWithEventDispatcher:(RCTEventDispatcher *)eventDispatcher{
    if (self=[super initWithEventDispatcher:eventDispatcher]) {
        ...
        //添加响应事件
        self.pan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panTouchUp:)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    //多列斜滑问题
    CGPoint newPoint = CGPointMake(0, scrollView.contentOffset.y);
    if ([[self.hotBlock objectForKey:@"lock"] isEqualToString:@"left"]) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer {
    // 点击 位置
    CGPoint pointXY = [gestureRecognizer locationInView:self.scrollView];
    CGFloat pointY = 0;
    
    if ([self.hotBlock objectForKey:@"hotBlock"]) {
        if ([[self.hotBlock objectForKey:@"hotBlock"] objectForKey:@"y"]) {
            pointY = [[[self.hotBlock objectForKey:@"hotBlock"] objectForKey:@"y"] floatValue];
        }
    }
    if (pointY < pointXY.y) {//没点 tableHeader
        return NO;
    } else {//点了 tableHeader
        return YES;
    }
...
- (void)panTouchUp:(UIPanGestureRecognizer *)gestureRecognizer {
    // 返回的是相对于最原始的手指的偏移量
    if ([self.hotBlock objectForKey:@"hotBlock"]) {
        if ([[self.hotBlock objectForKey:@"hotBlock"] objectForKey:@"y"]) {
            ...
            switch (gestureRecognizer.state) {
                case UIGestureRecognizerStateBegan:
                    self.startPoint = [gestureRecognizer translationInView:self.scrollView];
                    self.pointOffset = self.scrollView.contentOffset;
                    break;
                    
                case UIGestureRecognizerStateChanged:{
                    CGPoint curPoint = [gestureRecognizer translationInView:self.scrollView];
                    CGFloat distanceY = curPoint.y - self.startPoint.y;
                    if ([self shouldPulling]) {
                        self.scrollView.contentOffset = CGPointMake(self.scrollView.contentOffset.x, 0);
                    } else if ([self shouldDragging]) {
                        self.scrollView.contentOffset = CGPointMake(self.scrollView.contentOffset.x, self.scrollView.contentSize.height-self.scrollView.bounds.size.height+self.loadingFooterHeight);
                    } else {
                        self.scrollView.contentOffset = CGPointMake(self.scrollView.contentOffset.x, self.pointOffset.y+self.startPoint.y-distanceY);
                    }
                    break;
                }
                case UIGestureRecognizerStateEnded:{
                    ...
                    if (self.scrollView.contentOffset.y<0) {
                        [self.scrollView setContentOffset:CGPointMake(self.scrollView.contentOffset.x, 0) animated:YES];
                        [self sendScrollEventWithName:@"onRefresh" scrollView:self.scrollView userData:@{}];
                    } else if (self.scrollView.contentOffset.y>(self.scrollView.contentSize.height-self.scrollView.bounds.size.height+self.loadingFooterHeight)) {
                        self.scrollView.contentOffset = CGPointMake(self.pointOffset.x+self.startPoint.x, self.scrollView.contentSize.height-self.scrollView.bounds.size.height+self.loadingFooterHeight);
                    }
                    [gestureRecognizer setTranslation:CGPointZero inView:self.scrollView];
                    break;
                }
                default:
                    self.scrollView.contentOffset=self.pointOffset;
                    break;
            }
        }
    }
