...
//    }
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        FLog.e("VideoViewManager","isPortrait="+newConfig.orientation);

        if(newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE){
            isPortrait=false;
            if (videoWidth != 0 && videoHeight != 0) {
                mVideoContainer.setVideoSize(videoWidth, videoHeight);
                mVideoContainer.postInvalidate();
            }
//            mVideoContainer.setVideoSize(player.getVideoWidth(), player.getVideoHeight());

            FLog.e("VideoViewManager","isPortrait=false");
        }else{
            isPortrait=true;
            if (viewWidth != 0 && viewHeight != 0) {
                mVideoContainer.setVideoSize(viewWidth, viewHeight);
                mRootView.setMinimumWidth(viewWidth);
                mRootView.setMinimumHeight(viewHeight);
                mVideoContainer.postInvalidate();
            }
            FLog.e("VideoViewManager","isPortrait=true");
        }
    }
    ...
    IMediaPlayer.OnVideoSizeChangedListener onVideoSizeChangedListener = new IMediaPlayer.OnVideoSizeChangedListener() {
        ...
        public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sar_num, int sar_den) {
            ...
            viewWidth=LiveVideoView.super.getMeasuredWidth();
            viewHeight=LiveVideoView.super.getMeasuredHeight();

            FLog.e("VideoViewManager","videoWidth = "+videoWidth+"    videoHeight="+videoHeight);
            FLog.e("VideoViewManager","viewWidth = "+viewWidth+"    viewHeight="+viewHeight);
