...
class VodVideoView extends Component{
    ...
    pause(){
        UIManager.dispatchViewManagerCommand(
            ...
            UIManager.NativeHuoDeReplayVideo.Commands.pause,//Commands.pause与native层定义的COMMAND_PAUSE_NAME一致
            null//命令携带的参数数据
        );
    }
    ...
    resume(){
        UIManager.dispatchViewManagerCommand(
            findNodeHandle(this.refs[RCT_VIDEO_REF]),
            UIManager.NativeHuoDeReplayVideo.Commands.resume,
            null//命令携带的参数数据
        );
    }
    ...
    start(){
        UIManager.dispatchViewManagerCommand(
            findNodeHandle(this.refs[RCT_VIDEO_REF]),
            UIManager.NativeHuoDeReplayVideo.Commands.start,
            null
        );
    }
    ...
    leave(){
        UIManager.dispatchViewManagerCommand(
            findNodeHandle(this.refs[RCT_VIDEO_REF]),
            UIManager.NativeHuoDeReplayVideo.Commands.leave,
            null
        );  
    }
    ...
    init(){
        UIManager.dispatchViewManagerCommand(
            findNodeHandle(this.refs[RCT_VIDEO_REF]),
            UIManager.NativeHuoDeReplayVideo.Commands.init,
            null
        );  
    }
    seekTo(position){
        UIManager.dispatchViewManagerCommand(
            findNodeHandle(this.refs[RCT_VIDEO_REF]),
            UIManager.NativeHuoDeReplayVideo.Commands.seekTo,
            [position]
        );  
    }
    switch(seek){
        UIManager.dispatchViewManagerCommand(
            findNodeHandle(this.refs[RCT_VIDEO_REF]),
            UIManager.NativeHuoDeReplayVideo.Commands.switch,
            [seek]
        );  
    }
...
var RCTVodVideoView = requireNativeComponent('NativeHuoDeReplayVideo',VodVideoView,{
