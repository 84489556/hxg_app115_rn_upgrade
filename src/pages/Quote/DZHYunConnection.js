...
export default class DZHYunConnection {
  ...
  _createChannel() {
    ...
    // 创建https连接请求权限（访问令牌token） /oauth/token/access?appid=&secret_key=&deviceid=

    // 维护token的有效期

    // 带上token创建ws连接 ws://hostname:port/ws?token=xxxx

    // 用客户端的账号权限token到云平台验证并交换得到云平台账号token（用户相关信息需要在请求时带上，其实验证通过后用户权限应该和这个ws连接绑定，可以考虑不用每次请求时带上，是不是和上面的应用权限一样只要连接时带上就可以了呢，等UA实现后再考虑）
    ...
    try {
      ...
      if (request && request.isPause !== true) {
        ...
        if (!request.subscribe) {
          ...
          if (qid === undefined) {
            DeviceEventEmitter.emit('noMoreData')
          }
        
      }
    } catch(error) {
      console.debug("_onmessage catch:",error);
    }
  }
  ...
  getChannel() {
    ...
    return this._connectionPromise = this._connectionPromise || Promise.resolve().then(() => {
      ...
      // 如果_channel存在并且没有断掉而且token没有过期，则直接返回，否则创建新的连接
