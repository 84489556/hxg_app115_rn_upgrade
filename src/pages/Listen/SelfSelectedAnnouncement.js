...
export default class SelfSelectedAnnouncement extends YDNewsBasePage {

  static defaultProps = {
    serviceUrl: '/ydhxg/zixun/gonggao'
  };
            ...
            <View style={{flex:1, flexDirection:'row'}}>
              ...
              <View style={[styles.listItem, {flex:4}]}>
                  ...
                  <View style={styles.listItemFooter}>
                    <Text style={styles.listItemSource}>{item.sources}</Text>
  ...
  onPressItem(item) {
    ...
    if(Platform.OS === 'android' && item.link.substr(-4).toLowerCase() === '.pdf'){
      Navigation.pushForParams(this.props.navigation,'PDFPage',{news:data,title:item.title})
    }else{

      Navigation.pushForParams(this.props.navigation,'NewsDetailPage',{news:data,title:item.title})
    }
