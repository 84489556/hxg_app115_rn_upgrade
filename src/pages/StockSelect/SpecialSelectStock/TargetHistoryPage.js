...
export default class TargetHistoryPage extends BaseComponentPage<Props> {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            allLoaded :false,//没有更多数据了，本页默认为到底了,
        ...
        this.postPaths = HitsApi.TSZB_HISTORY_LIST;
    ...
    getHistory(callBack){
        ...
        if(this.state.titles[0].conCode!==0){
            params.sortField="intervalIncrease";
            ...
            params.sortField="intervalAmplitude";
            ...
            params.sortField="selectionDate";
        ...
        RequestInterface.baseGet(RequestInterface.HXG_BASE_URL,this.postPaths,params,
            (response)=>{
                ...
                if(response && response.length>0 && this.Page<=10){
                    for (let i =0;i<response.length;i++){
                        ...
                        titles.secCode = response[i].secCode;
                        newItem.title=titles;

                        //数据项，一定要按照数据添加
                        let dataItem=[];
                        dataItem.push(response[i].intervalIncrease!=0 ? Number(response[i].intervalIncrease):0);
                        dataItem.push(response[i].intervalAmplitude!=0 ? Number(response[i].intervalAmplitude):0);
                        dataItem.push(response[i].selectionDate ? response[i].selectionDate:'');
        ...
        return (
            <TouchableOpacity activeOpacity={0.6} onPress={()=>{
                ...
                <View style={styles.titleText}>
                    ...
                    <Text style={{fontSize:ScreenUtil.scaleSizeW(24),color:"#666666"}}>{item.title.secCode}</Text>
                </View>
       ...
       // let content = title;
        ...
        switch (index) {
            ...
            case 1:
                ...
                break;
            case 2:
                Views = <View key={index} style={{width:ScreenUtil.scaleSizeW(120),borderRadius:ScreenUtil.scaleSizeW(10),marginLeft:ScreenUtil.scaleSizeW(10),marginRight:ScreenUtil.scaleSizeW(15)
