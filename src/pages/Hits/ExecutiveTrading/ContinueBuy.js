...
//import NoRefreshWebChart from '../../../components/NoRefreshWebChart';
...
export default class FocusBuy extends Component<Props> {
    ...
    getFoucusBuyLine(){
        ...
        RequestInterface.baseGet(RequestInterface.HXG_BASE_URL,HitsApi.CHIXU_BUY_LINE,{},
            (response)=>{
    ...
    getFocusBugListData(){
        ...
        RequestInterface.baseGet(RequestInterface.HXG_BASE_URL,HitsApi.CHIXU_BUY_TABLEDATAS,params,
            (response)=>{
                //console.log("请求qian")
                ...
                if(response && response.list.length>0){
                    for (let i =0;i<response.list.length;i++){
                        ...
                        newItem.days = response.list[i].days ? response.list[i].days:'--';
    ...
    getSaleDetail(secCode){
        if(secCode && secCode!==""){
            ...
            //console.log("11111==="+secCode)
            this.state.detailsList = [];
            RequestInterface.baseGet(RequestInterface.HXG_BASE_URL,HitsApi.CHIXU_BUY_DETAILS,params,
                (response)=>{
                    if(response && response.length>0){
                        for (let i =0 ; i< (response.length<=3? response.length:3);i++){
                            this.state.detailsList.push(response[i])
                        }
                        this.setState({detailsList:this.state.detailsList});
    ...
    _renderItem = (path: IndexPath) => {
        ...
        return (
            <TouchableOpacity activeOpacity={1}  onPress={()=>{
                ...
                <View style={{width:ScreenUtil.screenW-ScreenUtil.scaleSizeW(60),height:ScreenUtil.scaleSizeW(110),
                    ...
                    <View style={{flex:1,height:ScreenUtil.scaleSizeW(90),flexDirection:"row",justifyContent:"center"}}>
                        <View style={{width:ScreenUtil.scaleSizeW(120),borderRadius:ScreenUtil.scaleSizeW(10),backgroundColor:"#fcf5ff",height:ScreenUtil.scaleSizeW(90),justifyContent:"center",alignItems:"center"}}>
                            <Text style={{fontSize:ScreenUtil.setSpText(40),color:"rgba(0,0,0,0.6)"}}>{item.days}</Text>
                        </View>
                    </View>
                    <View style={{flex:1,height:ScreenUtil.scaleSizeW(90),flexDirection:"row",justifyContent:"flex-end"}}>
                        <View style={{width:ScreenUtil.scaleSizeW(125),borderRadius:ScreenUtil.scaleSizeW(10),height:ScreenUtil.scaleSizeW(90)
                ...
                <View style={{width:ScreenUtil.screenW-ScreenUtil.scaleSizeW(60),height:ScreenUtil.scaleSizeW(120),marginLeft:ScreenUtil.scaleSizeW(30),flexDirection:"row",justifyContent:"center",alignItems:"center"
    ...
    _renderunLockHeader=()=>{
        ...
        return(
            <View style={{alignSelf:"stretch"}}>
                <View style={{width:ScreenUtil.screenW, height:ScreenUtil.scaleSizeW(790), justifyContent: "center", alignItems: "center",}}>
                    ...
                    <View style={styles.outView}>
                        ...
                        <View style={styles.itemAllView}>
                            <View style={styles.itemTitleView}>
                                ...
                                <TouchableOpacity onPress={()=>{Navigation.navigateForParams(this.props.navigation,"ChangeDetails",
                                    {keyWord: this.state.datasLineAll[this.state.saleDetailIndex].secName,
                                        keyWordCode: this.state.datasLineAll[this.state.saleDetailIndex].secCode })}}  style={{justifyContent:"center",alignItems:"center",flexDirection:"row"}}>
