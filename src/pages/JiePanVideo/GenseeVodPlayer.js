 ...
 * @Last Modified time: 2020-07-31 16:57:59
 * 展示回看播放器组件
 */

import React, { Component } from 'react';
import VideoView from '../Home/LiveLessionPage/ReplayVideoView';
import VideoPlayerView from '../Home/LiveLessionPage/IntradayPoint/liveView/NativeVodView';
import {
...
import { getHmsORms, get2HmsORms } from "../../utils/CommonUtils";
...
export default class GenseeVodPlayer extends Component {
    ...
    _toggleFullscreen() {
        ...
        this.props.onFullBtnPress && this.props.onFullBtnPress(state.isFullscreen);
        this.setState(state, () => {
        });
    }
    ...
    _togglePlayPause() {
        ...
        console.warn('VideoViewManager', 'state.paused=' + state.paused, 'this.state.showMobile=' + this.state.showMobile, 'currentTime=' + this.state.currentTime);
        this.setState(state, () => {
            if (!state.paused) {
                if (!this.state.showMobile && !this.state.error) {
                    if (Platform.OS == 'android') {
                        if (parseInt(this.state.currentTime) > 0) {
                            this.player.ref && this.player.ref.resume();
                        } else {
                            this.player.ref && this.player.ref.start();
                        }
                        ...
                        if (parseInt(this.state.currentTime) > 0) {
                            ...
                            this.player.ref && this.player.ref.play();
    ...
    _onBack() {
        if (!this.state.isFullscreen) {
            if (this.props.navigation) {
                this.releaseTimer();
                Platform.OS == 'ios' ? this.player.ref.release() : (this.player.ref && this.player.ref.leave());
    ...
    calculateTime() {
        if (this.state.showTimeRemaining) {
            ...
            return getHmsORms(time);
        }


        // return this.formatTime(this.state.duration / 1000);
        return getHmsORms(this.state.duration);
    }
    currentTime() {

        // return this.formatTime(this.state.currentTime / 1000);
        return get2HmsORms(this.state.currentTime, this.state.duration);
    }
    ...
    seekTo(time = 0) {
        ...
        this.player.ref && this.player.ref.seekTo(state.currentTime);
        this.setState(state);
    }
    ...
    componentDidMount() {
        ...
        if(Platform.OS == 'ios')
            !this.state.showMobile && !this.state.error && this.player.ref.play()
        else if (!this.state.showMobile && !this.state.error) {

            this.player.ref && this.player.ref.init();
            this.player.ref && this.player.ref.start();
        }
    ...
    switchUrl(url) {
        ...
        if (Platform.OS == "ios") {
            ...
            this.player.ref && this.player.ref.switch(url);
    ...
    _renderVideoView() {
        // console.log('this.props.videoPath===liveId======'+this.props.videoPath);
        // console.log('this.props.videoPath======nickName======'+UserInfoUtil.getUserId());
        if (Platform.OS == 'android') {
            return (
                <VideoView style={{ width: this.props.width, height: this.props.height }}
                    ref={videoPlayer => this.player.ref = videoPlayer}
                    //    source={{
                    //        domain: 'tzkt.gensee.com',
                    //        nickName: UserInfoUtil.getUserId() + '',
                    //        liveId:this.props.videoPath
                    //    }}
                    source={{
                        roomId: this.props.vodId,
                        ccId: this.props.userId,//'AE903AFD89036D43',
                        replayId: this.props.videoPath,
                        nickName: UserInfoUtil.getUserId() + '',
                        pwd: '',
                    }}
                    onPrepared={(duration) => {
                    ...
                    onProgress={(progress) => {
                        ...
                        //     if(parseInt(this.state.currentTime) >parseInt(progress)){
