 ...
 * @Last Modified time: 2020-07-31 16:56:52
 * 展示直播播放器组件
 */
...
import { Animated, Easing, Image, ImageBackground, PanResponder, Platform, StyleSheet, Text, TouchableOpacity, TouchableWithoutFeedback, View } from 'react-native';
import Orientation from 'react-native-orientation';
// import Slider from "react-native-slider";
import * as baseStyle from '../../components/baseStyle';
import UserInfoUtil from "../../utils/UserInfoUtil";
import LivePlayerView from '../Home/LiveLessionPage/IntradayPoint/liveView/NativeLiveView';
import LiveVideoView from '../Home/LiveLessionPage/LiveVideoView';
...
export default class GenseeLivePlayer extends Component {
    ...
    _togglePlayPause() {
        ...
        //console.log('VideoViewManager state.paused='+state.paused);
        this.setState(state, () => {
            if (!state.paused) {
                if (!this.state.showMobile && !this.state.error) {
                    //console.log('VideoViewManager this.player.ref.start()');
                    Platform.OS == 'ios' ? this.player.ref.play() : this.player.ref.start();
                }
                ...
                //console.log('VideoViewManager this.player.ref.leave()');
                // Platform.OS == 'ios' ?
    ...
    componentDidMount() {
        ...
        Platform.OS == 'ios' && !this.state.showMobile && !this.state.error && this.player.ref.play();
        Platform.OS == 'android' && !this.state.showMobile && !this.state.error && this.player.ref.start();
        sensorsDataClickObject.videoPlay.video_type = '直播视频'
    }
    ...
    componentWillUnmount() {
        this.releaseTimer();
        Platform.OS == 'ios' ? this.player.ref.release() : this.player.ref.leave();        
    }
    releaseTimer() {
        this.clearControlTimeout();
    ...
    _renderVideoView() {
        if (Platform.OS == 'android') {
            ...
            return (
                <LivePlayerView ref={videoPlayer => this.player.ref = videoPlayer}
                    style={[styles.player.video, this.styles.videoStyle]}
                    // vodId={this.props.vodId}
                    // dataSource={{
                    //     'domain': "tzkt.gensee.com",
                    //     'webcaseId': this.props.vodId,
                    //     'nickname': UserInfoUtil.getUserId() + '',
                    //     'password': ''
                    // }}
                    dataSource={{
                    ...
                    onCompletion={() => { this.props.videoFinish && this.props.videoFinish(); }}
                />
            )
        }
    }
