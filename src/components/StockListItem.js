...
import { StyleSheet, View, Text, TouchableHighlight, Animated, Dimensions, Platform } from 'react-native';
...
export default class StockListItem extends BaseComponent {
            ...
            return (
                <View key="ZhongWenJianCheng" style={this.getStyles('stockLabel')}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <View><StockFormatText style={this.getStyles('name')}>{name}</StockFormatText></View>
                        <View style={[this.getStyles('name'), { justifyContent: 'center', padding: DISTANCE(12), height: DISTANCE(32), marginLeft: DISTANCE(10), backgroundColor: baseStyle.BLACK_30, borderRadius: 4 }]}>
                            <View style={{}}><Text style={{ color: baseStyle.WHITE, fontSize: RATE(20) }}>退</Text></View>
                        </View>
                    </View>
    ...
    _renderZuiXinJia() {
        return (
            <View key="ZuiXinJia" style={[this.getStyles('priceContainer'), this.props.ZuiXinJiaStyle]}>
                <StockFormatText titlename={"ZuiXinJia"}
                    style={this.getUpDownStyle('price', this.props.ZhangFu)}>{this.props.ZuiXinJia}</StockFormatText>
            </View>
        );
    }

    _renderZhangDie() {
        return <StockFormatText key="ZhangDie" style={[this.getUpDownStyle('rise', this.props.ZhangDie), { textAlign: 'right', flex: 1, }]}
            sign={true}>{this.props.ZhangDie}</StockFormatText>;
    }

    _renderZhangFu() {
        return <StockFormatText key="ZhangFu" style={[this.getUpDownStyle('rise', this.props.ZhangFu), { width: deviceWidth < 350 ? 80 : 100, }]} unit="%"
            sign={true}>{this.props.ZhangFu / 100}</StockFormatText>;
    }

    _renderZhenFu() {
        return (<StockFormatText key="ZhenFu" unit="%" style={{
            textAlign: 'center',
            fontSize: 16,
            marginLeft: 20,
            width: 100
        }}>{this.props.ZhenFu / 100}</StockFormatText>);
    }

    _renderHuanShou() {
        return (<StockFormatText key="HuanShou" unit="%" style={{
            textAlign: 'center',
            fontSize: RATE(32),
            color: baseStyle.BLACK_100,
            marginLeft: 20,
            width: 100
        }}>{this.props.HuanShou / 100}</StockFormatText>);
    }
    ...
    render() {
        ...
        return this.props.onPress ? (
            <TouchableHighlight
                ...
                onLongPress={() => this.props.onLongPress && this.props.onLongPress(this.props)}
